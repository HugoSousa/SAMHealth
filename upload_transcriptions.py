...
for patient_folder in os.listdir(os.getcwd() + "/transcriptions"):
	...
	with open(info_file) as info_json:
		...
		for session in sessions:
			...
			file_name = file_name.replace(" ", "%20") #replace white spaces with %20
			doc_id = patient + "_" + session_number

			dparams = '-Dparams=literal.id=' + doc_id + '&literal.patient=' + patient + '&literal.therapist=' + therapist + '&literal.session_number=' + session_number + '&literal.file=' + file_name
